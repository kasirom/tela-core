...
#[derive(Parser)]
#[command(name = "tela")]
#[command(about = "Compiler Tela Core (Get-Touch-Tela)", long_about = None)]
struct Cli {
    #[command(subcommand)]
    command: Commands,
}
...
fn main() {
    ...
    match &cli.command {
        ...
        Commands::Bangun { target } => {
            let target_clone = target.clone();
            let handle = std::thread::Builder::new()
                .stack_size(8 * 1024 * 1024)
                .spawn(move || {
                    bangun_proyek(false, false, target_clone);
                })
                .unwrap();
            handle.join().unwrap();
        }
        Commands::Jalankan { target } => {
            ...
            let handle = std::thread::Builder::new()
                .stack_size(8 * 1024 * 1024)
                .spawn(move || {
                    bangun_proyek(true, false, target_clone);
                })
                .unwrap();
            handle.join().unwrap();
        }
        Commands::Uji { target } => {
            ...
            let handle = std::thread::Builder::new()
                .stack_size(8 * 1024 * 1024)
                .spawn(move || {
                    bangun_proyek(true, true, target_clone);
                })
                .unwrap();
            handle.join().unwrap();
        }
...
fn bangun_proyek(jalankan: bool, mode_uji: bool, target: Option<String>) {
    ...
    println!("⚙️  Membaca src/utama.tela...");
    ...
    println!("⚙️  Menjalankan Analisis Semantik...");
    ...
    println!("⚙️  Menghasilkan Kode LLVM IR...");
    ...
    println!("⚙️  Memanggil Clang untuk kompilasi executable...");
