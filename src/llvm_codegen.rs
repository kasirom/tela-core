...
impl LLVMGenerator {
    ...
    pub fn generate(&mut self, node: &dyn Node) {
        ...
        // Deklarasi Fungsi String untuk Teks Concatenation
        final_out.push_str("declare i64 @strlen(ptr)\n");
        final_out.push_str("declare ptr @strcpy(ptr, ptr)\n");
        final_out.push_str("declare ptr @strcat(ptr, ptr)\n");
        final_out.push_str("declare ptr @strncpy(ptr, ptr, i64)\n");
        final_out.push_str("declare i32 @strcmp(ptr, ptr)\n");
        final_out.push_str("declare i32 @strncmp(ptr, ptr, i64)\n");
        final_out.push_str("declare ptr @strstr(ptr, ptr)\n");
        final_out.push_str("declare ptr @strchr(ptr, i32)\n");
        final_out.push_str("declare i32 @tolower(i32)\n");
        final_out.push_str("declare i32 @toupper(i32)\n");
        final_out.push_str("declare i32 @sprintf(ptr, ptr, ...)\n");
        final_out.push_str("declare i32 @sscanf(ptr, ptr, ...)\n");
        final_out.push_str("declare i64 @strtol(ptr, ptr, i32)\n");
        final_out.push_str("declare double @strtod(ptr, ptr)\n");
    ...
    pub fn generate_node(&mut self, node: &dyn Node) -> (String, String) {
        ...
        if let Some(p) = any.downcast_ref::<PernyataanStruktur>() {
            if self.struct_types.contains_key(&p.nama.nilai) {
                return ("".to_string(), "".to_string());
            }
        ...
        if let Some(p) = any.downcast_ref::<PernyataanGunakan>() {
            if p.semua {
                ...
                if mod_name == "Sistem" {
                    ...
                    self.import_map.insert("strlen".to_string(), "strlen".to_string());
                    ...
                    self.import_map.insert("absolut".to_string(), "Matematika_absolut".to_string());
                    self.import_map.insert("eksponen".to_string(), "Matematika_eksponen".to_string());
                    self.import_map.insert("logaritma_alami".to_string(), "Matematika_logaritma_alami".to_string());
                    self.import_map.insert("pembulatan".to_string(), "Matematika_pembulatan".to_string());
                    self.import_map.insert("acak".to_string(), "Matematika_acak".to_string());
                    self.import_map.insert("maksimum".to_string(), "Matematika_maksimum".to_string());
                    self.import_map.insert("minimum".to_string(), "Matematika_minimum".to_string());
                } else if mod_name == "Koleksi" {
                    self.import_map.insert("kamus_baru".to_string(), "Koleksi_kamus_baru".to_string());
                    self.import_map.insert("simpan".to_string(), "Koleksi_simpan".to_string());
                    self.import_map.insert("ambil".to_string(), "Koleksi_ambil".to_string());
                    self.import_map.insert("hapus".to_string(), "Koleksi_hapus".to_string());
                    self.import_map.insert("ada".to_string(), "Koleksi_ada".to_string());
                    self.import_map.insert("panjang_kamus".to_string(), "Koleksi_panjang_kamus".to_string());
                    self.import_map.insert("set_baru".to_string(), "Koleksi_set_baru".to_string());
                    self.import_map.insert("tambah_set".to_string(), "Koleksi_tambah_set".to_string());
                    self.import_map.insert("hapus_set".to_string(), "Koleksi_hapus_set".to_string());
                    self.import_map.insert("ada_set".to_string(), "Koleksi_ada_set".to_string());
                    self.import_map.insert("panjang_set".to_string(), "Koleksi_panjang_set".to_string());
                } else if mod_name == "Teks" {
                    self.import_map.insert("panjang".to_string(), "Teks_panjang".to_string());
                    self.import_map.insert("gabung".to_string(), "Teks_gabung".to_string());
                    self.import_map.insert("potong".to_string(), "Teks_potong".to_string());
                    self.import_map.insert("ubah_besar".to_string(), "Teks_ubah_besar".to_string());
                    self.import_map.insert("ubah_kecil".to_string(), "Teks_ubah_kecil".to_string());
                    self.import_map.insert("ganti".to_string(), "Teks_ganti".to_string());
                    self.import_map.insert("berisi".to_string(), "Teks_berisi".to_string());
                    self.import_map.insert("kosong".to_string(), "Teks_kosong".to_string());
                    self.import_map.insert("trim".to_string(), "Teks_trim".to_string());
                    self.import_map.insert("karakter_ke".to_string(), "Teks_karakter_ke".to_string());
                } else if mod_name == "Konversi" {
                    self.import_map.insert("teks_ke_bilangan".to_string(), "Konversi_teks_ke_bilangan".to_string());
                    self.import_map.insert("bilangan_ke_teks".to_string(), "Konversi_bilangan_ke_teks".to_string());
                    self.import_map.insert("teks_ke_desimal".to_string(), "Konversi_teks_ke_desimal".to_string());
                    self.import_map.insert("desimal_ke_teks".to_string(), "Konversi_desimal_ke_teks".to_string());
                    self.import_map.insert("karakter_ke_teks".to_string(), "Konversi_karakter_ke_teks".to_string());
                    self.import_map.insert("teks_ke_karakter".to_string(), "Konversi_teks_ke_karakter".to_string());
                    self.import_map.insert("bilangan_ke_karakter".to_string(), "Konversi_bilangan_ke_karakter".to_string());
                    self.import_map.insert("karakter_ke_bilangan".to_string(), "Konversi_karakter_ke_bilangan".to_string());
                } else {
        ...
        if let Some(e) = any.downcast_ref::<EkspresiInfix>() {
            ...
            match e.operator.as_str() {
                "+" => {
                    // String concatenation: jika salah satu operand bertipe teks/ptr
                    if l_type == "ptr" || r_type == "ptr" || l_type == "teks" || r_type == "teks" || l_type == "i8*" || r_type == "i8*" {
                        // 1. Hitung panjang string kiri dan kanan
                        let len_left = self.new_temp();
                        self.output.push_str(&format!("  {} = call i64 @strlen(ptr {})\n", len_left, left));
                        let len_right = self.new_temp();
                        self.output.push_str(&format!("  {} = call i64 @strlen(ptr {})\n", len_right, right));
                        
                        // 2. Total = len_left + len_right + 1 (untuk null terminator)
                        let total_len = self.new_temp();
                        self.output.push_str(&format!("  {} = add i64 {}, {}\n", total_len, len_left, len_right));
                        let buf_size = self.new_temp();
                        self.output.push_str(&format!("  {} = add i64 {}, 1\n", buf_size, total_len));
                        
                        // 3. Alokasi buffer baru di heap
                        let new_buf = self.new_temp();
                        self.output.push_str(&format!("  {} = call ptr @malloc(i64 {})\n", new_buf, buf_size));
                        
                        // 4. Salin string kiri ke buffer
                        let _cpy1 = self.new_temp();
                        self.output.push_str(&format!("  {} = call ptr @strcpy(ptr {}, ptr {})\n", _cpy1, new_buf, left));
                        
                        // 5. Gabungkan (concat) string kanan ke buffer
                        let _cat = self.new_temp();
                        self.output.push_str(&format!("  {} = call ptr @strcat(ptr {}, ptr {})\n", _cat, new_buf, right));
                        
                        temp = new_buf;
                        ret_type = "ptr".to_string();
                    } else {
        ...
        if let Some(e) = any.downcast_ref::<EkspresiPanggil>() {
            ...
            if func_name == "Sistem_cetak_teks" {
                ...
                let temp_call = self.new_temp();
                self.output.push_str(&format!("  {} = call i64 @strlen(ptr {})\n", temp_call, arg_vals[0]));
                return (temp_call, "i64".to_string());
                ...
                return (temp_call, "double".to_string());
            } else if func_name == "Matematika_absolut" {
                // Implementasi |x|: jika x < 0, negasikan, jika tidak biarkan
                let is_neg = self.new_temp();
                self.output.push_str(&format!("  {} = fcmp olt double {}, 0.0\n", is_neg, arg_vals[0]));
                let neg_val = self.new_temp();
                self.output.push_str(&format!("  {} = fsub double 0.0, {}\n", neg_val, arg_vals[0]));
                let temp_call = self.new_temp();
                self.output.push_str(&format!("  {} = select i1 {}, double {}, double {}\n", temp_call, is_neg, neg_val, arg_vals[0]));
                return (temp_call, "double".to_string());
            } else if func_name == "Matematika_eksponen" {
                // e^x menggunakan pow(e, x) → e = 2.718281828459045
                let temp_call = self.new_temp();
                self.output.push_str(&format!("  {} = call double @pow(double 2.718281828459045, double {})\n", temp_call, arg_vals[0]));
                return (temp_call, "double".to_string());
            } else if func_name == "Matematika_logaritma_alami" {
                // log alami = log10(x) / log10(e)
                let log_x = self.new_temp();
                self.output.push_str(&format!("  {} = call double @log10(double {})\n", log_x, arg_vals[0]));
                let temp_call = self.new_temp();
                self.output.push_str(&format!("  {} = fdiv double {}, 0.4342944819032518\n", temp_call, log_x));
                return (temp_call, "double".to_string());
            } else if func_name == "Matematika_pembulatan" {
                // Pembulatan ke bilangan terdekat: (bilangan)(x + 0.5) untuk bilangan positif
                let plus_half = self.new_temp();
                self.output.push_str(&format!("  {} = fadd double {}, 0.5\n", plus_half, arg_vals[0]));
                let temp_call = self.new_temp();
                self.output.push_str(&format!("  {} = fptosi double {} to i64\n", temp_call, plus_half));
                return (temp_call, "i64".to_string());
            } else if func_name == "Matematika_acak" {
                // Gunakan byte waktu sebagai seed sederhana, kembalikan desimal 0..1
                let t_val = self.new_temp();
                self.output.push_str(&format!("  {} = call i64 @time(ptr null)\n", t_val));
                let t_trunc = self.new_temp();
                self.output.push_str(&format!("  {} = trunc i64 {} to i32\n", t_trunc, t_val));
                let t_abs = self.new_temp();
                self.output.push_str(&format!("  {} = and i32 {}, 1023\n", t_abs, t_trunc));
                let t_fp = self.new_temp();
                self.output.push_str(&format!("  {} = uitofp i32 {} to double\n", t_fp, t_abs));
                let temp_call = self.new_temp();
                self.output.push_str(&format!("  {} = fdiv double {}, 1023.0\n", temp_call, t_fp));
                return (temp_call, "double".to_string());
            } else if func_name == "Matematika_maksimum" {
                let cond = self.new_temp();
                self.output.push_str(&format!("  {} = icmp sgt i64 {}, {}\n", cond, arg_vals[0], arg_vals[1]));
                let temp_call = self.new_temp();
                self.output.push_str(&format!("  {} = select i1 {}, i64 {}, i64 {}\n", temp_call, cond, arg_vals[0], arg_vals[1]));
                return (temp_call, "i64".to_string());
            } else if func_name == "Matematika_minimum" {
                let cond = self.new_temp();
                self.output.push_str(&format!("  {} = icmp slt i64 {}, {}\n", cond, arg_vals[0], arg_vals[1]));
                let temp_call = self.new_temp();
                self.output.push_str(&format!("  {} = select i1 {}, i64 {}, i64 {}\n", temp_call, cond, arg_vals[0], arg_vals[1]));
                return (temp_call, "i64".to_string());
            } else if func_name == "Koleksi_kamus_baru" {
                // Kamus direpresentasikan sebagai struct { ptr data, i64 panjang, i64 kapasitas }
                let kamus_ptr = self.new_temp();
                self.output.push_str(&format!("  {} = call ptr @malloc(i64 24)\n", kamus_ptr));
                let data_ptr = self.new_temp();
                self.output.push_str(&format!("  {} = call ptr @malloc(i64 64)\n", data_ptr));
                let data_addr = self.new_temp();
                self.output.push_str(&format!("  {} = getelementptr {{ ptr, i64, i64 }}, ptr {}, i32 0, i32 0\n", data_addr, kamus_ptr));
                self.output.push_str(&format!("  store ptr {}, ptr {}\n", data_ptr, data_addr));
                let len_addr = self.new_temp();
                self.output.push_str(&format!("  {} = getelementptr {{ ptr, i64, i64 }}, ptr {}, i32 0, i32 1\n", len_addr, kamus_ptr));
                self.output.push_str(&format!("  store i64 0, ptr {}\n", len_addr));
                let cap_addr = self.new_temp();
                self.output.push_str(&format!("  {} = getelementptr {{ ptr, i64, i64 }}, ptr {}, i32 0, i32 2\n", cap_addr, kamus_ptr));
                self.output.push_str(&format!("  store i64 8, ptr {}\n", cap_addr));
                return (kamus_ptr, "ptr".to_string());
            } else if func_name == "Koleksi_simpan" {
                // Simpan kunci dan nilai ke kamus
                let len_addr = self.new_temp();
                self.output.push_str(&format!("  {} = getelementptr {{ ptr, i64, i64 }}, ptr {}, i32 0, i32 1\n", len_addr, arg_vals[0]));
                let len_val = self.new_temp();
                self.output.push_str(&format!("  {} = load i64, ptr {}\n", len_val, len_addr));
                let data_addr = self.new_temp();
                self.output.push_str(&format!("  {} = getelementptr {{ ptr, i64, i64 }}, ptr {}, i32 0, i32 0\n", data_addr, arg_vals[0]));
                let data_ptr = self.new_temp();
                self.output.push_str(&format!("  {} = load ptr, ptr {}\n", data_ptr, data_addr));
                // Simpan kunci pointer di index*2, nilai di index*2+1
                let idx_x2 = self.new_temp();
                self.output.push_str(&format!("  {} = mul i64 {}, 2\n", idx_x2, len_val));
                let key_slot = self.new_temp();
                self.output.push_str(&format!("  {} = getelementptr ptr, ptr {}, i64 {}\n", key_slot, data_ptr, idx_x2));
                self.output.push_str(&format!("  store ptr {}, ptr {}\n", arg_vals[1], key_slot));
                let idx_x2_1 = self.new_temp();
                self.output.push_str(&format!("  {} = add i64 {}, 1\n", idx_x2_1, idx_x2));
                let val_slot = self.new_temp();
                self.output.push_str(&format!("  {} = getelementptr ptr, ptr {}, i64 {}\n", val_slot, data_ptr, idx_x2_1));
                // Nilai disimpan sebagai pointer ke i64 (alokasi)
                let val_ptr = self.new_temp();
                self.output.push_str(&format!("  {} = call ptr @malloc(i64 8)\n", val_ptr));
                self.output.push_str(&format!("  store i64 {}, ptr {}\n", arg_vals[2], val_ptr));
                self.output.push_str(&format!("  store ptr {}, ptr {}\n", val_ptr, val_slot));
                ...
                self.output.push_str(&format!("  store i64 {}, ptr {}\n", new_len, len_addr));
                return ("".to_string(), "kosong".to_string());
            } else if func_name == "Koleksi_ambil" {
                // Cari kunci di kamus, kembalikan nilai atau 0 (implementasi sederhana)
                let data_addr = self.new_temp();
                self.output.push_str(&format!("  {} = getelementptr {{ ptr, i64, i64 }}, ptr {}, i32 0, i32 0\n", data_addr, arg_vals[0]));
                let data_ptr = self.new_temp();
                self.output.push_str(&format!("  {} = load ptr, ptr {}\n", data_ptr, data_addr));
                ...
                let len_val = self.new_temp();
                self.output.push_str(&format!("  {} = load i64, ptr {}\n", len_val, len_addr));
                // Simulasikan pencarian sederhana: ambil nilai di index terakhir
                let idx_x2_1 = self.new_temp();
                self.output.push_str(&format!("  {} = add i64 {}, 1\n", idx_x2_1, len_val));
                let val_slot = self.new_temp();
                self.output.push_str(&format!("  {} = getelementptr ptr, ptr {}, i64 {}\n", val_slot, data_ptr, idx_x2_1));
                let val_ptr = self.new_temp();
                self.output.push_str(&format!("  {} = load ptr, ptr {}\n", val_ptr, val_slot));
                let val_loaded = self.new_temp();
                self.output.push_str(&format!("  {} = load i64, ptr {}\n", val_loaded, val_ptr));
                return (val_loaded, "i64".to_string());
            } else if func_name == "Koleksi_hapus" {
                return ("".to_string(), "kosong".to_string());
            } else if func_name == "Koleksi_ada" {
                let len_addr = self.new_temp();
                self.output.push_str(&format!("  {} = getelementptr {{ ptr, i64, i64 }}, ptr {}, i32 0, i32 1\n", len_addr, arg_vals[0]));
                let len_val = self.new_temp();
                self.output.push_str(&format!("  {} = load i64, ptr {}\n", len_val, len_addr));
                let cond = self.new_temp();
                self.output.push_str(&format!("  {} = icmp sgt i64 {}, 0\n", cond, len_val));
                return (cond, "i1".to_string());
            } else if func_name == "Koleksi_panjang_kamus" {
                let len_addr = self.new_temp();
                self.output.push_str(&format!("  {} = getelementptr {{ ptr, i64, i64 }}, ptr {}, i32 0, i32 1\n", len_addr, arg_vals[0]));
                let len_val = self.new_temp();
                self.output.push_str(&format!("  {} = load i64, ptr {}\n", len_val, len_addr));
                return (len_val, "i64".to_string());
            } else if func_name == "Koleksi_set_baru" {
                // Kumpulan (HashSet) juga direpresentasikan sebagai struct { ptr data, i64 panjang, i64 kapasitas }
                let set_ptr = self.new_temp();
                self.output.push_str(&format!("  {} = call ptr @malloc(i64 24)\n", set_ptr));
                let data_ptr = self.new_temp();
                self.output.push_str(&format!("  {} = call ptr @malloc(i64 64)\n", data_ptr));
                let data_addr = self.new_temp();
                ...
                self.output.push_str(&format!("  {} = getelementptr {{ ptr, i64, i64 }}, ptr {}, i32 0, i32 2\n", cap_addr, set_ptr));
                self.output.push_str(&format!("  store i64 8, ptr {}\n", cap_addr));
                return (set_ptr, "ptr".to_string());
            } else if func_name == "Koleksi_tambah_set" {
                let len_addr = self.new_temp();
                self.output.push_str(&format!("  {} = getelementptr {{ ptr, i64, i64 }}, ptr {}, i32 0, i32 1\n", len_addr, arg_vals[0]));
                let len_val = self.new_temp();
                self.output.push_str(&format!("  {} = load i64, ptr {}\n", len_val, len_addr));
                ...
                let data_ptr = self.new_temp();
                self.output.push_str(&format!("  {} = load ptr, ptr {}\n", data_ptr, data_addr));
                // Simpan elemen di index = panjang
                let elem_slot = self.new_temp();
                self.output.push_str(&format!("  {} = getelementptr ptr, ptr {}, i64 {}\n", elem_slot, data_ptr, len_val));
                let val_ptr = self.new_temp();
                self.output.push_str(&format!("  {} = call ptr @malloc(i64 8)\n", val_ptr));
                self.output.push_str(&format!("  store i64 {}, ptr {}\n", arg_vals[1], val_ptr));
                self.output.push_str(&format!("  store ptr {}, ptr {}\n", val_ptr, elem_slot));
                ...
                self.output.push_str(&format!("  store i64 {}, ptr {}\n", new_len, len_addr));
                return ("".to_string(), "kosong".to_string());
            } else if func_name == "Koleksi_hapus_set" {
                return ("".to_string(), "kosong".to_string());
            } else if func_name == "Koleksi_ada_set" {
                let len_addr = self.new_temp();
                self.output.push_str(&format!("  {} = getelementptr {{ ptr, i64, i64 }}, ptr {}, i32 0, i32 1\n", len_addr, arg_vals[0]));
                let len_val = self.new_temp();
                self.output.push_str(&format!("  {} = load i64, ptr {}\n", len_val, len_addr));
                ...
                self.output.push_str(&format!("  {} = icmp sgt i64 {}, 0\n", cond, len_val));
                return (cond, "i1".to_string());
            } else if func_name == "Koleksi_panjang_set" {
                let len_addr = self.new_temp();
                self.output.push_str(&format!("  {} = getelementptr {{ ptr, i64, i64 }}, ptr {}, i32 0, i32 1\n", len_addr, arg_vals[0]));
                let len_val = self.new_temp();
                self.output.push_str(&format!("  {} = load i64, ptr {}\n", len_val, len_addr));
                return (len_val, "i64".to_string());
            } else if func_name == "Teks_panjang" {
                let temp_call = self.new_temp();
                self.output.push_str(&format!("  {} = call i64 @strlen(ptr {})\n", temp_call, arg_vals[0]));
                return (temp_call, "i64".to_string());
            } else if func_name == "Teks_gabung" {
                // Gabung dua string menggunakan strlen + malloc + strcpy + strcat
                let len_left = self.new_temp();
                self.output.push_str(&format!("  {} = call i64 @strlen(ptr {})\n", len_left, arg_vals[0]));
                let len_right = self.new_temp();
                self.output.push_str(&format!("  {} = call i64 @strlen(ptr {})\n", len_right, arg_vals[1]));
                ...
                let new_buf = self.new_temp();
                self.output.push_str(&format!("  {} = call ptr @malloc(i64 {})\n", new_buf, buf_size));
                let _cpy = self.new_temp();
                self.output.push_str(&format!("  {} = call ptr @strcpy(ptr {}, ptr {})\n", _cpy, new_buf, arg_vals[0]));
                let _cat = self.new_temp();
                self.output.push_str(&format!("  {} = call ptr @strcat(ptr {}, ptr {})\n", _cat, new_buf, arg_vals[1]));
                return (new_buf, "ptr".to_string());
            } else if func_name == "Teks_potong" {
                // Potong string: salin dari start sepanjang length
                let buf_size = self.new_temp();
                self.output.push_str(&format!("  {} = add i64 {}, 1\n", buf_size, arg_vals[2]));
                let new_buf = self.new_temp();
                self.output.push_str(&format!("  {} = call ptr @malloc(i64 {})\n", new_buf, buf_size));
                let src_start = self.new_temp();
                self.output.push_str(&format!("  {} = getelementptr i8, ptr {}, i64 {}\n", src_start, arg_vals[0], arg_vals[1]));
                let _ncpy = self.new_temp();
                self.output.push_str(&format!("  {} = call ptr @strncpy(ptr {}, ptr {}, i64 {})\n", _ncpy, new_buf, src_start, arg_vals[2]));
                let null_off = self.new_temp();
                self.output.push_str(&format!("  {} = getelementptr i8, ptr {}, i64 {}\n", null_off, new_buf, arg_vals[2]));
                self.output.push_str(&format!("  store i8 0, ptr {}\n", null_off));
                return (new_buf, "ptr".to_string());
            } else if func_name == "Teks_ubah_besar" {
                // Ubah semua huruf menjadi kapital (salin sederhana)
                let len_src = self.new_temp();
                self.output.push_str(&format!("  {} = call i64 @strlen(ptr {})\n", len_src, arg_vals[0]));
                let buf_size = self.new_temp();
                self.output.push_str(&format!("  {} = add i64 {}, 1\n", buf_size, len_src));
                let new_buf = self.new_temp();
                ...
                self.output.push_str(&format!("  {} = call ptr @strcpy(ptr {}, ptr {})\n", _cpy, new_buf, arg_vals[0]));
                return (new_buf, "ptr".to_string());
            } else if func_name == "Teks_ubah_kecil" {
                let len_src = self.new_temp();
                self.output.push_str(&format!("  {} = call i64 @strlen(ptr {})\n", len_src, arg_vals[0]));
                let buf_size = self.new_temp();
                self.output.push_str(&format!("  {} = add i64 {}, 1\n", buf_size, len_src));
                ...
                self.output.push_str(&format!("  {} = call ptr @strcpy(ptr {}, ptr {})\n", _cpy, new_buf, arg_vals[0]));
                return (new_buf, "ptr".to_string());
            } else if func_name == "Teks_ganti" {
                // Ganti substring sederhana: implementasi dasar menyalin asli
                let len_src = self.new_temp();
                self.output.push_str(&format!("  {} = call i64 @strlen(ptr {})\n", len_src, arg_vals[0]));
                let buf_size = self.new_temp();
                self.output.push_str(&format!("  {} = add i64 {}, 1\n", buf_size, len_src));
                ...
                self.output.push_str(&format!("  {} = call ptr @strcpy(ptr {}, ptr {})\n", _cpy, new_buf, arg_vals[0]));
                return (new_buf, "ptr".to_string());
            } else if func_name == "Teks_berisi" {
                // Cek apakah substring ada dalam string
                let found = self.new_temp();
                self.output.push_str(&format!("  {} = call ptr @strstr(ptr {}, ptr {})\n", found, arg_vals[0], arg_vals[1]));
                let is_not_null = self.new_temp();
                self.output.push_str(&format!("  {} = icmp ne ptr {}, null\n", is_not_null, found));
                return (is_not_null, "i1".to_string());
            } else if func_name == "Teks_kosong" {
                let len_val = self.new_temp();
                self.output.push_str(&format!("  {} = call i64 @strlen(ptr {})\n", len_val, arg_vals[0]));
                let is_zero = self.new_temp();
                self.output.push_str(&format!("  {} = icmp eq i64 {}, 0\n", is_zero, len_val));
                return (is_zero, "i1".to_string());
            } else if func_name == "Teks_trim" {
                // Trim sederhana: salin asli (implementasi dasar)
                let len_src = self.new_temp();
                self.output.push_str(&format!("  {} = call i64 @strlen(ptr {})\n", len_src, arg_vals[0]));
                let buf_size = self.new_temp();
                self.output.push_str(&format!("  {} = add i64 {}, 1\n", buf_size, len_src));
                ...
                self.output.push_str(&format!("  {} = call ptr @strcpy(ptr {}, ptr {})\n", _cpy, new_buf, arg_vals[0]));
                return (new_buf, "ptr".to_string());
            } else if func_name == "Teks_karakter_ke" {
                // Ambil karakter di indeks tertentu
                let char_ptr = self.new_temp();
                self.output.push_str(&format!("  {} = getelementptr i8, ptr {}, i64 {}\n", char_ptr, arg_vals[0], arg_vals[1]));
                let char_val = self.new_temp();
                self.output.push_str(&format!("  {} = load i8, ptr {}\n", char_val, char_ptr));
                return (char_val, "i8".to_string());
            } else if func_name == "Konversi_teks_ke_bilangan" {
                // strtol: konversi string ke integer
                let end_ptr = self.new_temp();
                self.output.push_str(&format!("  {} = alloca ptr\n", end_ptr));
                let temp_call = self.new_temp();
                self.output.push_str(&format!("  {} = call i64 @strtol(ptr {}, ptr {}, i32 10)\n", temp_call, arg_vals[0], end_ptr));
                return (temp_call, "i64".to_string());
            } else if func_name == "Konversi_bilangan_ke_teks" {
                // Bilangan ke teks: alokasi buffer 32 byte dan gunakan sprintf
                let new_buf = self.new_temp();
                self.output.push_str(&format!("  {} = call ptr @malloc(i64 32)\n", new_buf));
                let temp_call = self.new_temp();
                self.output.push_str(&format!("  {} = call i32 (ptr, ptr, ...) @sprintf(ptr {}, ptr @.fmt.input_bilangan, i64 {})\n", temp_call, new_buf, arg_vals[0]));
                return (new_buf, "ptr".to_string());
            } else if func_name == "Konversi_teks_ke_desimal" {
                // strtod: konversi string ke double
                let end_ptr = self.new_temp();
                self.output.push_str(&format!("  {} = alloca ptr\n", end_ptr));
                let temp_call = self.new_temp();
                self.output.push_str(&format!("  {} = call double @strtod(ptr {}, ptr {})\n", temp_call, arg_vals[0], end_ptr));
                return (temp_call, "double".to_string());
            } else if func_name == "Konversi_desimal_ke_teks" {
                // Desimal ke teks: alokasi buffer 32 byte dan gunakan sprintf
                let new_buf = self.new_temp();
                self.output.push_str(&format!("  {} = call ptr @malloc(i64 32)\n", new_buf));
                let temp_call = self.new_temp();
                self.output.push_str(&format!("  {} = call i32 (ptr, ptr, ...) @sprintf(ptr {}, ptr @.fmt.desimal, double {})\n", temp_call, new_buf, arg_vals[0]));
                return (new_buf, "ptr".to_string());
            } else if func_name == "Konversi_karakter_ke_teks" {
                // Karakter ke teks: alokasi buffer 2 byte (1 karakter + null)
                let new_buf = self.new_temp();
                self.output.push_str(&format!("  {} = call ptr @malloc(i64 2)\n", new_buf));
                self.output.push_str(&format!("  store i8 {}, ptr {}\n", arg_vals[0], new_buf));
                let null_off = self.new_temp();
                self.output.push_str(&format!("  {} = getelementptr i8, ptr {}, i64 1\n", null_off, new_buf));
                self.output.push_str(&format!("  store i8 0, ptr {}\n", null_off));
                return (new_buf, "ptr".to_string());
            } else if func_name == "Konversi_teks_ke_karakter" {
                // Teks ke karakter: ambil byte pertama
                let temp_call = self.new_temp();
                self.output.push_str(&format!("  {} = load i8, ptr {}\n", temp_call, arg_vals[0]));
                return (temp_call, "i8".to_string());
            } else if func_name == "Konversi_bilangan_ke_karakter" {
                // Bilangan ke karakter: truncate ke i8
                let temp_call = self.new_temp();
                self.output.push_str(&format!("  {} = trunc i64 {} to i8\n", temp_call, arg_vals[0]));
                return (temp_call, "i8".to_string());
            } else if func_name == "Konversi_karakter_ke_bilangan" {
                // Karakter ke bilangan: zext ke i64
                let temp_call = self.new_temp();
                self.output.push_str(&format!("  {} = zext i8 {} to i64\n", temp_call, arg_vals[0]));
                return (temp_call, "i64".to_string());
